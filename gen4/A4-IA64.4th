...
: JMP  1 jmpcall ;                    ( ... )
: ?JMP ( target cond )  2 !args !condition .code 70 + #BYTE there 2+ reladdr  nip op, ;
: LAHF  9F op00 ;                     ( )
: LAR  02 oplp ;                      ( r/m reg )
...
: LOCK  F0 op00 ;                     ( )
: LODS  AC strop ;                    ( size )
: LOOP ( target )  1 !args !immediate drop  #BYTE there 2+ reladdr E2 op, ;
: ?LOOP ( target cond )  2 !args ![n]eq E0 + swap !immediate drop #BYTE there 2+ reladdr op, ;
...
: NOP ( [n] )  op#@ unless  1 #  then  1 !args  drop
  ...
  T.ARCH i16 = if  imm@ 1- if  NOP#_NOT_SUPPORTED$ error cleanup exit  then  90 tc,  else  imm@
    1=?if  drop  90 tc,  cleanup exit  then
    2=?if  drop  66 tc, 90 tc,  cleanup exit  then
    3=?if  drop  0F tc, 1F tc, 00 tc,  cleanup exit  then
    4=?if  drop  0F tc, 1F tc, 40 tc, 00 tc,  cleanup exit  then
    5=?if  drop  0F tc, 1F tc, 44 tc, 00 tc, 00 tc,  cleanup exit  then
    6=?if  drop  66 tc, 0F tc, 1F tc, 44 tc, 00 tc, 00 tc,  cleanup exit  then
    7=?if  drop  0F tc, 1F tc, 80 tc, 00 tc, 00 tc, 00 tc, 00 tc,  cleanup exit  then
    8=?if  drop  0F tc, 1F tc, 84 tc, 00 tc, 00 tc, 00 tc, 00 tc, 00 tc,  cleanup exit  then
    9=?if  drop  66 tc, 0F tc, 1F tc, 84 tc, 00 tc, 00 tc, 00 tc, 00 tc, 00 tc,  cleanup exit  then
...
: ?JMPFWD ( cond -- &a )  $0F tc, $80 or tc, there 0 td, ;
( Inserts an unconditional jump forward and returns referent &a of the location to relocate. )
: JMPFWD ( -- &a ) $E9 tc, there 0 td, ;
: ?JMPBCK ( cond &a -- )  16 assertSpace  there 2+ delta dup -128 128 within simblock? andn if
  $70 rot $0F and + tc, tc,  else  $0F tc, $80 rot $0F and + tc, 4- td,  then ;
: JMPBCK ( &a -- )  16 assertSpace  there 2+ delta dup -128 128 within simblock? andn if
  $EB tc, tc,  else  $E9 tc, 3− td,  then ;
: resolve ( &a -- )  there over delta 4− swap d! ;

( [a], [a1], [a2] denote referents on the A-stack. )

: (BEGIN) ( -- [a] )  there >A  cleanup ;
: (END) ( [a] -- )  A> drop  cleanup ;
: (AGAIN) ( [a] -- )  A> JMPBCK cleanup ;
: (UNTIL) ( cond [a] -- )  1 xor A> ?JMPBCK  cleanup ;
: (ASLONG) ( cond [a] -- )  A> swap ?JMPBCK  cleanup ;
: (IF) ( cond -- [a] )  1 xor ?JMPFWD >A  cleanup ;
...
: (FOR) ( -- [a] )  (BEGIN) ;
: (NEXT) ( [a] -- )  there 2+ A@ delta -128 128 within simblock? andn if
  A> # LOOP  else  RCX DEC  CNZ A> ?JMPBCK then  cleanup ;
: (NEXTIF) ( cond [a] -- )  there 2+ A@ delta -128 128 within simblock? andn if
