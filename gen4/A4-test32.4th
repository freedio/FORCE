vocabulary A4-test64
  ...
  requires" IA64-ForcemblerTools.voc"
  requires" IA64-Forcembler.voc"

64 dup dup constant _ADDRSIZE constant _OPSIZE constant _ARCH
-1 constant _MMX  -1 constant _XMM  -1 constant _X87

variable ERRORS
create EXPECTED_ERROR$  256 0allot
create ACTUAL_ERROR$  256 0allot

: !op ( op code arch mode size type -- )
  3pick 4u<< or 4u<< 4 roll arch>adsize or 4u<< or 8u<< or 8u<< or =??unless
    2dup swap 2 "Opcode composition failed: %08XH should be %08XH" asmerror..  ERRORS 1+!  then
  2drop ;
: ¶ ( -- )  cleanup  INLINE 1+!  ERRMSG @ if  asmerror.  ERRORS 1+!  then ;
...
: !error ( e$ -- )  ERRMSG @ unless  clerinc sourceFile@ sourceColumn@ sourceLine@ 4 roll
    ...
    ERRMSG @ ≠??if
      ...
      "», but got «". err- $. err+ "»!". err-  then  then  sp0 sp! ;
: !disp ( n # -- )  dispsize@ ≠??if
    2dup swap 2 "Expected displacement size %d, but got %d" asmerror.. then  2drop
  disp@ ≠??if  2dup swap 2 "Expected displacement %d, but got %d" asmerror.. then  2drop ;
...
:: testsuite ( -- )  INIT,
  ...
  8 TARGET-ADDRSIZE !
  0 0 ERRHANDLER 2!
  "A4-test64.4th" INFILENAME !  0 INCOLUMN !  40 INLINE !

  RAX                     00 #IA64 #REGULAR #QWORD #REGISTER !op    ¶
  DS                      03 #IALL #SEGMENT #WORD #REGISTER !op     ¶
  CL                      01 #IALL #REGULAR #BYTE #REGISTER !op     ¶
  DH                      06 #IALL #HI-BYTE #BYTE #REGISTER !op     ¶
  XMM7                    07 #IXMM #XMM #OWORD #REGISTER !op        ¶
  MM4                     04 #IMMX #MMX #QWORD #REGISTER !op        ¶
  CR8                     08 #IA64 #CONTROL #DWORD #REGISTER !op    ¶
  DR2                     02 #IA32 #DEBUG #DWORD #REGISTER !op      ¶
  R09                     09 #IA64 #REGULAR #QWORD #REGISTER !op    ¶
  R13D                    13 #IA64 #REGULAR #DWORD #REGISTER !op    ¶
  ST(3)                   03 #IFPU #FLOATING #TBYTE #REGISTER !op   ¶
¶
  0 [EAX]                 INVALID_ADDRESS_SIZE$ !error                                     ¶
  0 [RAX]                 00 #IA64 #LINEAR 0 #ADDRESS !op            0 0 !disp             ¶
  -1 [BP+SI]              02 #IA16 #INDEXED 0 #ADDRESS !op           -1 #BYTE !disp        ¶
  100 []                  $45 #IA64 #INDEXED %MOD00 + 0 #ADDRESS !op 100 #DWORD !disp      ¶
  0 [RBX] [ESI]           ( TARGET_ARCHITECTURE_MISMATCH$ !error )                             ¶
/*
  ...
  ~ 10 # AL ADC                     { 14 10 }
  ~ 10 # AX ADC                     { 66 15 10 00 }
  ~ 10 # EAX ADC                    { 15 10 00 00 00 }
  ~ 10 # RAX ADC                    { 48 15 10 00 00 00 }
  ~ AL AH ADC                       { 10 C4 }
  ...
  ~ HERE 100 + # U< ?JMP            TARGET_ADDRESS_OUT_OF_RANGE$ !error
  ~ HERE 10 + # JMP                 { EB 0E }
  ~ HERE 10 + # NEAR JMP            { E9 0B 00 00 00 }
  ~ HERE 1000 + # JMP               { E9 FB 0F 00 00 }
  ...
  ~ XSETBV                          { 0F 01 D1 }
  ~ NOP BEGIN  RAX RAX ADD  END  NOP   { 90 EB 05 E9 03 00 00 00 48 01 C0 90 }
  ~ NOP BEGIN  RAX RAX SUB  AGAIN  NOP    { 90 EB 05 E9 05 00 00 00 48 29 C0 EB FB 90 }
  ~ NOP BEGIN  RAX RAX XOR  U< UNTIL  NOP    { 90 EB 05 E9 06 00 00 00 48 31 C0 3E 73 FA 90 }
  ~ NOP BEGIN  RAX NOT  simblock+ U< UNTIL  NOP
    { 90 EB 05 E9 0B 00 00 00 48 F7 D0 2E 72 05 E9 F5 FF FF FF 90 }
  ~ NOP > IF  RAX RAX AND  THEN  NOP
    { 90 7F 05 E9 03 00 00 00 48 21 C0 90 }
  ~ NOP < UNLESS  RAX RAX OR  ELSE  RAX RAX AND  THEN  NOP
    { 90 7D 05 E9 08 00 00 00 48 09 C0 E9 03 00 00 00 48 21 C0 90 }
  ~ NOP > IFEVER  RAX RAX AND  THEN  NOP
    { 90 2E 7F 05 E9 03 00 00 00 48 21 C0 90 }
  ~ NOP < UNLESSEVER  RAX RAX OR  ELSE  RAX RAX AND  THEN  NOP
    { 90 2E 7D 05 E9 08 00 00 00 48 09 C0 E9 03 00 00 00 48 21 C0 90 }
  ~ NOP BEGIN  RAX RAX TEST  0> WHILE  RAX DEC  REPEAT  NOP
    { 90 EB 05 E9 10 00 00 00 48 85 C0 3E 7F 05 E9 05 00 00 00 48 FF C8 EB F0 90 }
  ~ NOP  FOR  RAX RAX TEST  NEXT  NOP   { 90 EB 05 E9 05 00 00 00 48 85 C0 E2 FB 90 }
  ~ NOP  FOR  RAX RAX TEST  simblock+ NEXT  NOP
    { 90 EB 05 E9 0D 00 00 00 48 85 C0 48 FF C9 74 05 E9 F3 FF FF FF 90 }
  ...
  ~ CELL [RBP] RBP LEA  QWORD PTR -CELL [RBP] POP              { 48 8D 6D 08 8F 45 F8 }
  ~ QWORD PTR -CELL [RBP] PUSH  -CELL [RBP] RBP LEA  RET       { FF 75 F8 48 8D 6D F8 C3 }
  */
