/** Forcembler Base for Intel IA-64 */

vocabulary AsmBase-IA64
  requires" FORTH.voc"
  requires" IO.voc"
  requires" OS.voc"
...
: .line  sourceFile@ sourceColumn@ 1+ sourceLine@ 1+ 3 "At %d.%d in «%s»: "|! ;
...
: error ( $ -- )  AsmBase-IA64 ERROR-HANDLER @ execute ;
...
: A@  ASTACK ASP @ + cell- @ ;
: A1@  ASTACK ASP @ + 2 cells - @ ;
: A2@  ASTACK ASP @ + 3 cells - @ ;
...
: !=opsize|0 ( op sz -- op )  over .size = over .size 0= or unless  INVALID_OPERAND_SIZE$ error  then ;
( assert that operand sizes are equal )
: !opsize= ( op1 op2 -- op1 op2 )  check-opsize  supply-opsize  !opsize
  over .size over .size -  if  OPERAND_SIZE_MISMATCH$ error  then
...
: makeDirect ( &r code mode size arch -- addr )
  #ADDRESS 4u<< + 4u<< + 8u<< + 8u<< + swap reloc! reloc+ 0 disp! addr+ >sizex ;
...
: disp, ( -- )  dispsize@
      0 =?if  drop  exit  then
  #BYTE =if   disp@ #BYTE #punch  exit  then
  disp@ addrsize? if  addrsize@ else T.ADDRSIZE  then  #punch ;
: modregr/m, ( ... -- )  modregr/m? if  modregr/m@ c,  sib? if  sib@ c,  then  disp, then ;
: opsize, ( -- )
  opsize? fpuop? andn if
    opsize@ #QWORD = if  REX.W+  then
    opsize@ #WORD = T.ADDRSIZE #WORD ≠ and if  $66 c,  then then ;
: addrsize, ( -- )  addrsize? if  addrsize@ T.ADDRSIZE = unless  $67 c,  then then ;
: prefix, ( -- )  prefix? if  prefix@ c,  then ;
: rex, ( -- )
  rex? fpuop? andn if rex@ 8= mmxop? and unless rex@ mmxop? 8and andn $40+ c, then then ;
: escape, ( -- )  esc? if  $0F c,  then ;
: prefices, ( -- )  opsize, addrsize, prefix, rex, escape, ;
: imm, ( -- )  imm? if  imm@ immsize@  wide? if  ##punch  else  #punch  then then ;
: operands, ( -- )  modregr/m, imm, ;
: validate ( -- )  hibyte? rex? and if  HIBYTE_NOT_AVAILABLE_WITH_REX$ error  then ;
: op, ( ... opc -- )  validate prefices, c, operands,  cleanup ;
: op&, ( ... opc -- )  validate prefices, c, operands, ;
: 2op, ( ... opc2 opc1 -- )  validate prefices, swap c, c, operands,  cleanup ;
: daop, ( opc -- )  validate prefices, c,  disp@ T.ARCH ##punch  cleanup ;
...
: flop01  0 !args  !fpu $D9 c, c, cleanup ;
...
: pushpopr/m ( r/m opc )
  dup 6* reg! $70* $8F+ swap op>opsize! T.ADDRSIZE !=opsize|0 noREX+ stdmodr/m! op, ;
