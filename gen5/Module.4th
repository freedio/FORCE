...
: loadModule ( m$ -- #v )  locateModule  debug? if  cr dup 1 "Loading module «%s»"|log  then
  ...
  discardFile  targetVoc# dup resolveDependencies  dup applyRelocations  dup voc>dict
  debug? if  dup 1 " as vocabulary#%d"|log  then
  debug? if  dup #vocabulary$ cr 1 "Running %s.init"|log  then  dup runInits
  dup targetVoc! ;
